...
- Name: trade/spotMarketBuyOrder.spec.ts >> Spot Module — Market Buy Order Positive Flow >> TC-03c: order book view switches (all / sell-only / buy-only) @sanity
- Location: tests/trade/spotMarketBuyOrder.spec.ts:121:5
...
```ts
  ...
  99  |     test('TC-02b: 24h ticker header matches Binance reference data (exact match)', async () => {
...
> 126 |         expect.soft(sellOb.topAsk, 'Sell-only view: ask price should be positive').toBeGreaterThan(0);
  ...
  211 |         const { topAsk, topBid } = await spotMarketBuyPage.getOrderBookTopBidAsk();
  ...
  222 |             spotMarketBuyPage.getOrderBookTopBidAsk(),
  223 |             BinanceHelper.getOrderBook(page, getTradeData().searchPair, 5),
  224 |         ]);
  225 |         const binanceBid = ob.bids[0]?.price ?? 0;
  226 |         const binanceAsk = ob.asks[0]?.price ?? 0;
```