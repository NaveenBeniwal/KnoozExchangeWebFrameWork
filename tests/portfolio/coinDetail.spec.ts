...
test('coin detail page shows coin name, Overview tab and Primary balance tab @smoke @sanity', async () => {
...
test('coin detail page shows a live price and a price change percentage @smoke @sanity', async () => {
    for (const item of coinData) {
        await coinDetailPage.goToCoinDetail(item.name);
...
test('all five chart time period buttons are visible @sanity', async () => {
...
test('market stats section with all four stat labels is visible @sanity', async () => {
    for (const item of coinData) {
        await coinDetailPage.goToCoinDetail(item.name);
...
test('overview text section is visible and content matches expected text from CSV @sanity', async () => {
    for (const item of coinData) {
        await coinDetailPage.goToCoinDetail(item.name);
