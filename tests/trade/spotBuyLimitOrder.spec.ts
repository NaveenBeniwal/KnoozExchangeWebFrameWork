...
// Actual total from All Orders — used in TC-19 for balance check
...
test.describe.serial('Spot Module — Buy Limit Order & Open Orders Validation', () => {
    ...
    test.beforeAll(async ({ playwright }, testInfo) => {
        ...
        await spotBuyPage.markAsFavorite(tradeData.searchPair).catch(() => {});
        await spotBuyPage.unmarkFavorite(tradeData.searchPair).catch(() => {});
    });
    ...
    test('TC-01: select currency pair from ALL tab @sanity', async () => {
        await spotBuyPage.selectCurrencyPair(tradeData.searchPair);
        console.log('[TC-01] Currency pair selected from ALL tab');
    });

    // ── TC-02 ─────────────────────────────────────────────────────────────────
    test('TC-02: precision change alters LTP decimal display and restores cleanly @sanity', async () => {
        ...
        console.log(`[TC-02] Precision toggle | Original: "${origPrec}" | Alternate: "${altPrec}" | LTP After Restore: ${ltpAfter}`);
    });

    // ── TC-03 ─────────────────────────────────────────────────────────────────
    test('TC-03: clicking order book ask rows (5 rows) pre-fills Price and Amount inputs @sanity', async () => {
        ...
        console.log(`[TC-03] Summary: ${passCount}/${rows.length} ask rows matched price+amount in form`);
    });

    // ── TC-04 ─────────────────────────────────────────────────────────────────
    test('TC-04: Buy button label shows "BUY {baseCoin}" dynamically @sanity', async () => {
        ...
        console.log(`[TC-04] Buy button label | Label: "${label}"`);
    });

    // ── TC-05 ─────────────────────────────────────────────────────────────────
    test('TC-05: Sell tab shows base currency available balance @sanity', async () => {
        ...
        console.log(`[TC-05] ${baseCoin} available balance on Sell tab | Available Balance: ${sellAvlb}`);
    });

    // ── TC-06–TC-09: Percentage buttons ───────────────────────────────────────
    test('TC-06: 25% button fills the amount with 25% of available balance @sanity', async () => {
        // TC-05 leaves us on sell tab — switch back to buy+limit before testing percentage buttons
        await spotBuyPage.selectLimitBuyTab();
        ...
        console.log(`[TC-06] 25% Button | Available Balance: ${avlb} | Total Field: ${filled} | Diff%: ${avlb > 0 && expected > 0 ? diffPct(filled, expected) : 'N/A'}`);
    });

    test('TC-07: 50% button fills the amount with 50% of available balance @sanity', async () => {
        const avlb     = snapshotBeforeOrder?.buyAvlb ?? await spotBuyPage.fetchAvailableBalance();
        const price    = parseFloat(tradeData.buyLimitPrice);
        ...
        console.log(`[TC-07] 50% Button | Available Balance: ${avlb} | Total Field: ${filled} | Diff%: ${avlb > 0 && expected > 0 ? diffPct(filled, expected) : 'N/A'}`);
    });

    test('TC-08: 75% button fills the amount with 75% of available balance @sanity', async () => {
        const avlb     = snapshotBeforeOrder?.buyAvlb ?? await spotBuyPage.fetchAvailableBalance();
        const price    = parseFloat(tradeData.buyLimitPrice);
        ...
        console.log(`[TC-08] 75% Button | Available Balance: ${avlb} | Total Field: ${filled} | Diff%: ${avlb > 0 && expected > 0 ? diffPct(filled, expected) : 'N/A'}`);
    });

    test('TC-09: 100% button fills the amount with 100% of available balance @sanity', async () => {
        const avlb     = snapshotBeforeOrder?.buyAvlb ?? await spotBuyPage.fetchAvailableBalance();
        const price    = parseFloat(tradeData.buyLimitPrice);
        ...
        console.log(`[TC-09] 100% Button | Available Balance: ${avlb} | Total Field: ${filled} | Diff%: ${avlb > 0 && expected > 0 ? diffPct(filled, expected) : 'N/A'}`);
    });

    // ── TC-10 ─────────────────────────────────────────────────────────────────
    test('TC-10: capture full balance snapshot before placing order @sanity', async () => {
        ...
        if (r.buyMatchPortfolio !== null) {
            expect.soft(r.buyMatchPortfolio, `TC-10 Buy avlb ${r.snapshot.buyAvlb} should match portfolio ${quoteCoin} spot ${r.portfolioQuoteBalance}`).toBe(true);
        }
        if (r.sellMatchPortfolio !== null) {
            expect.soft(r.sellMatchPortfolio, `TC-10 Sell avlb ${r.snapshot.sellAvlb} should match portfolio ${baseCoin} spot ${r.portfolioBaseBalance}`).toBe(true);
        }
        console.log(`[TC-10] ${r.log}`);
    });

    // ── TC-11 ─────────────────────────────────────────────────────────────────
    test('TC-11: enter limit price and total, verify estimated fee matches UI @sanity', async () => {
        const price = parseFloat(tradeData.buyLimitPrice);
        ...
        console.log(`[TC-11] Price: ${price} | Total Entered: ${total} | Calculated Amount: ${r.calculatedAmount} | FeePercent: ${feePercent} | Est Fee: ${r.estFee.toFixed(8)} | UI Est Fee: ${r.uiEstFee.toFixed(8)} | Fee Match: ${r.feeMatchStatus}`);
    });
    ...
    test('TC-12: Total field auto-calculates as price × amount @sanity', async () => {
        ...
        if (amount > 0 && v.actual > 0 && totalField > 0) {
            expect.soft(
                ...
                `TC-12 Total field diff% — field:${totalField} expected(price×amount):${price * amount}`,
            ).toBeLessThan(1);
        } else {
            expect.soft(totalField, 'Total field should be non-negative').toBeGreaterThanOrEqual(0);
        }
        console.log(`[TC-12] Total field auto-calc | Price: ${price} | Amount: ${amount} | Total Field: ${totalField} | Diff%: ${amount > 0 && totalField > 0 ? diffPct(totalField, price * amount) : 'N/A'}`);
    });

    // ── TC-13 ─────────────────────────────────────────────────────────────────
    test('TC-13: fetch available balance before placing order @sanity', async () => {
        availableBalanceBefore = snapshotBeforeOrder?.buyAvlb ?? await spotBuyPage.fetchAvailableBalance();
        console.log(`[TC-13] Balance before order: ${availableBalanceBefore}`);
    });

    // ── TC-14 ─────────────────────────────────────────────────────────────────
    test('TC-14: confirm buy order and verify success message', async () => {
        ...
        console.log(`[TC-14] Confirmed buy limit order | Pair: ${orderDetails.pair} | Executed Price: ${orderDetails.price} | Executed Amount: ${orderDetails.amount} | OrderId: ${orderId} | Success Message: "${r.successMessage || '(none)'}"`)
    });

    // ── TC-15 ─────────────────────────────────────────────────────────────────
    test('TC-15: balance snapshot after placing order — pending lock or immediate fill', async () => {
        ...
        console.log(`[TC-15] Balance snapshot (${r.label}) | Buy Avlb Before: ${snapshotBeforeOrder.buyAvlb} | Buy Avlb After: ${r.afterSnap.buyAvlb}`);
    });

    // ── TC-16 ─────────────────────────────────────────────────────────────────
    test('TC-16: Open Orders section shows "Open Orders" tab and "View All" button', async () => {
    ...
    // ── TC-17 ─────────────────────────────────────────────────────────────────
    test('TC-17: latest open order row matches the placed order details', async () => {
        ...
        if (orderFilledImmediately) {
            test.info().annotations.push({ type: 'suggestion', description: `TC-17: Limit price (${tradeData.buyLimitPrice}) was above market (${marketPriceAtOrder}) — order filled immediately, no pending entry in Open Orders.` });
    ...
    // ── TC-18 ─────────────────────────────────────────────────────────────────
    test('TC-18: latest entry in All Orders tab matches the placed order details', async () => {
        ...
        console.log(`[TC-18] All Orders (${orderFilledImmediately ? 'immediate fill' : 'pending'}) | Pair: ${r.pairActual} | Type: ${r.typeActual} | Side: ${r.sideActual} | Limit Price: ${r.priceFieldActual} | Status: ${r.statusActual} | Filled: ${r.filledActual} | Remaining: ${r.remainingActual} | Total: ${r.totalActual} | OrderId: ${r.orderId}`);
    });

    // ── TC-19 ─────────────────────────────────────────────────────────────────
    test('TC-19: available balance decreases by the order total after placing the order', async () => {
        ...
        expect.soft(v.actual,      `TC-19: All Orders total must not exceed placed total (${orderDetails.total})`).toBeLessThanOrEqual(orderDetails.total * 1.001);
        expect.soft(v.pctBelowOk ? v.pctBelow : `fail: ${v.pctBelow.toFixed(2)}% below`, `TC-19: total diff% must be < 6%`).toBeLessThan(6);
        ...
        console.log(`[TC-19] Balance check | Input total: ${orderDetails.total} | All Orders total: ${v.actual} | ${v.pctBelow.toFixed(2)}% below | Balance status: "${r.balanceValidStatus}"`);
    });

    // ── TC-20 ─────────────────────────────────────────────────────────────────
    test('TC-20: Trade History tab shows the placed order (skipped if pending)', async () => {
    ...
    // ── TC-21 ─────────────────────────────────────────────────────────────────
    test('TC-21: Trade History bottom tab shows the placed order', async () => {
        ...
        if (!entry) { console.warn('[TC-21] Trade History returned no rows'); return; }
    ...
    // ── TC-22 ─────────────────────────────────────────────────────────────────
    test('TC-22: cancel latest open order and verify balance is fully restored', async () => {
        test.skip(!orderSucceeded, 'Order not placed — insufficient balance or order failed. Nothing to cancel.');
        if (orderFilledImmediately) {
            test.info().annotations.push({ type: 'suggestion', description: `TC-22: Limit price (${tradeData.buyLimitPrice}) was above market (${marketPriceAtOrder}) — order filled immediately, nothing to cancel.` });
            console.log('[TC-22] Order filled immediately — no pending order to cancel');
            return;
        }
        const r = await spotBuyPage.cancelLatestOrderAndVerifyBalance(tradeData.searchPair, 'buy', orderDetails.orderId);
        ...
        console.log(`[TC-22] Cancel order | Cancelled Status: "${r.cancelledStatus}" | Balance Restored Status: "${r.balanceRestoredStatus}" | Message: "${r.cancelledMsg}"`);
    });

    // ── TC-23 ─────────────────────────────────────────────────────────────────
    test('TC-23: full balance snapshot after cancel confirms quote coin fully restored', async () => {
        ...
        if (!snapshotAfterOrder) { console.warn('[TC-23] Post-order snapshot missing'); return; }
        const results = await spotBuyPage.validateBalanceAfterOrderCancel(
            portfolioSpotPage, snapshotAfterOrder, quoteCoin, baseCoin,
            orderDetails.amount, orderDetails.price, 'buy', tradeData.searchPair,
        );
        for (const r of results) expect.soft(r.pass, r.msg).toBe(true);
        console.log(`[TC-23] Balance after cancel | Buy Avlb After Order: ${snapshotAfterOrder.buyAvlb}`);
    });
    ...
    test('TC-24: snapshot before placing multiple pending orders (Cancel All test)', async () => {
        ...
        console.log(`[TC-24] Pre-multi-order snapshot | Buy Avlb: ${snapshotBeforeMultiOrders.buyAvlb} | Sell Avlb: ${snapshotBeforeMultiOrders.sellAvlb}`);
    });

    // ── TC-25: 1st pending buy limit order ─────────────────────────────────
    test('TC-25: place 1st pending buy limit order and verify state after placement', async () => {
        ...
        test.skip(!price2 || !total2, 'buyLimitPrice2 or buyLimitTotal2 missing in CSV — skipping TC-25.');
        ...
        expect.soft(r.successMsg,  'TC-25: 1st pending buy order should succeed').toContain('success');
        expect.soft(r.totalOk  ? 'pass'  :`fail(${r.actual} > ${total2 * 1.001})`,  `TC-25: All Orders total must not exceed entered (${total2})`).toBe('pass');
        expect.soft(r.pctBelowOk ? 'pass' :`fail(${r.pctBelow.toFixed(2)}%)`,       `TC-25: total diff% must be < 6%`).toBe('pass');
        ...
        expect.soft(c.balanceOk ? 'pass' : `fail(avlb=${c.balanceNow} exp≈${c.expectedBalance} diff=${c.balanceDiff})`, 'TC-25: buyAvlb should decrease by locked amount').toBe('pass');
        console.log(`[TC-25] 1st Pending Buy | Price: ${price2} | Total: ${r.actual} | Open: ${c.openCount} | buyAvlb: ${c.balanceNow} | OrderId: ${r.orderId}`);
    });

    // ── TC-26: 2nd pending buy limit order ─────────────────────────────────
    test('TC-26: place 2nd pending buy limit order and verify state after placement', async () => {
        ...
        test.skip(!price3 || !total3, 'buyLimitPrice3 or buyLimitTotal3 missing in CSV — skipping TC-26.');
        const r = await spotBuyPage.placePendingBuyLimitOrder(price3, total3);
        multiOrderActualBuyTotals.push(r.actual);
        expect.soft(r.successMsg,  'TC-26: 2nd pending buy order should succeed').toContain('success');
        expect.soft(r.totalOk  ? 'pass'  :`fail(${r.actual} > ${total3 * 1.001})`,  `TC-26: All Orders total must not exceed entered (${total3})`).toBe('pass');
        expect.soft(r.pctBelowOk ? 'pass' :`fail(${r.pctBelow.toFixed(2)}%)`,       `TC-26: total diff% must be < 6%`).toBe('pass');
        ...
        expect.soft(c.balanceOk ? 'pass' : `fail(avlb=${c.balanceNow} exp≈${c.expectedBalance} diff=${c.balanceDiff})`, 'TC-26: buyAvlb should decrease by locked amount').toBe('pass');
        console.log(`[TC-26] 2nd Pending Buy | Price: ${price3} | Total: ${r.actual} | Open: ${c.openCount} | buyAvlb: ${c.balanceNow} | OrderId: ${r.orderId}`);
    });

    // ── TC-27: 3rd pending buy limit order ─────────────────────────────────
    test('TC-27: place 3rd pending buy limit order and verify state after placement', async () => {
        ...
        test.skip(!price4 || !total4, 'buyLimitPrice4 or buyLimitTotal4 missing in CSV — skipping TC-27.');
        const r = await spotBuyPage.placePendingBuyLimitOrder(price4, total4);
        multiOrderActualBuyTotals.push(r.actual);
        expect.soft(r.successMsg,  'TC-27: 3rd pending buy order should succeed').toContain('success');
        expect.soft(r.totalOk  ? 'pass'  :`fail(${r.actual} > ${total4 * 1.001})`,  `TC-27: All Orders total must not exceed entered (${total4})`).toBe('pass');
        expect.soft(r.pctBelowOk ? 'pass' :`fail(${r.pctBelow.toFixed(2)}%)`,       `TC-27: total diff% must be < 6%`).toBe('pass');
        ...
        expect.soft(c.balanceOk ? 'pass' : `fail(avlb=${c.balanceNow} exp≈${c.expectedBalance} diff=${c.balanceDiff})`, 'TC-27: buyAvlb should decrease by locked amount').toBe('pass');
        console.log(`[TC-27] 3rd Pending Buy | Price: ${price4} | Total: ${r.actual} | Open: ${c.openCount} | buyAvlb: ${c.balanceNow} | OrderId: ${r.orderId}`);
    });

    // ── TC-28: 4th pending buy limit order ─────────────────────────────────
    test('TC-28: place 4th pending buy limit order and verify state after placement', async () => {
        ...
        test.skip(!price5 || !total5, 'buyLimitPrice5 or buyLimitTotal5 missing in CSV — skipping TC-28.');
        const r = await spotBuyPage.placePendingBuyLimitOrder(price5, total5);
        multiOrderActualBuyTotals.push(r.actual);
        expect.soft(r.successMsg,  'TC-28: 4th pending buy order should succeed').toContain('success');
        expect.soft(r.totalOk  ? 'pass'  :`fail(${r.actual} > ${total5 * 1.001})`,  `TC-28: All Orders total must not exceed entered (${total5})`).toBe('pass');
        expect.soft(r.pctBelowOk ? 'pass' :`fail(${r.pctBelow.toFixed(2)}%)`,       `TC-28: total diff% must be < 6%`).toBe('pass');
        ...
        expect.soft(c.balanceOk ? 'pass' : `fail(avlb=${c.balanceNow} exp≈${c.expectedBalance} diff=${c.balanceDiff})`, 'TC-28: buyAvlb should decrease by locked amount').toBe('pass');
        console.log(`[TC-28] 4th Pending Buy | Price: ${price5} | Total: ${r.actual} | Open: ${c.openCount} | buyAvlb: ${c.balanceNow} | OrderId: ${r.orderId}`);
    });

    // ── TC-29: 1st pending sell limit order (for cancel-all) ───────────────
    test('TC-29: place 1st pending sell limit order and verify state after placement', async () => {
        ...
        test.skip(!sellPrice1 || !sellAmount1, 'cancelAllSellPrice1 or cancelAllSellAmount1 missing in CSV — skipping TC-29.');
        ...
        expect.soft(r.successMsg,  'TC-29: 1st pending sell order should succeed').toContain('success');
        expect.soft(r.amountOk ? 'pass' :`fail(${r.actual} > ${sellAmount1 * 1.001})`, `TC-29: All Orders remaining must not exceed entered (${sellAmount1})`).toBe('pass');
        expect.soft(r.pctBelowOk ? 'pass' :`fail(${r.pctBelow.toFixed(2)}%)`,          `TC-29: amount diff% must be < 6%`).toBe('pass');
        ...
        expect.soft(c.tradeHistoryExcluded ? 'not-in-th' : 'in-th',                        `TC-29: Pending sell order ${r.orderId} should NOT be in Trade History`).toBe('not-in-th');
        expect.soft(c.balanceOk ? 'pass' : `fail(avlb=${c.balanceNow} exp≈${c.expectedBalance} diff=${c.balanceDiff})`, 'TC-29: sellAvlb should decrease by locked amount').toBe('pass');
        console.log(`[TC-29] 1st Pending Sell | Price: ${sellPrice1} | Amount: ${r.actual} | Open: ${c.openCount} | sellAvlb: ${c.balanceNow} | OrderId: ${r.orderId}`);
    });

    // ── TC-30: 2nd pending sell limit order (for cancel-all) ───────────────
    test('TC-30: place 2nd pending sell limit order and verify state after placement', async () => {
        ...
        test.skip(!sellPrice2 || !sellAmount2, 'cancelAllSellPrice2 or cancelAllSellAmount2 missing in CSV — skipping TC-30.');
        const r = await spotBuyPage.placePendingSellLimitOrderForBuySpec(sellPrice2, sellAmount2);
        multiOrderActualSellAmounts.push(r.actual);
        expect.soft(r.successMsg,  'TC-30: 2nd pending sell order should succeed').toContain('success');
        expect.soft(r.amountOk ? 'pass' :`fail(${r.actual} > ${sellAmount2 * 1.001})`, `TC-30: All Orders remaining must not exceed entered (${sellAmount2})`).toBe('pass');
        expect.soft(r.pctBelowOk ? 'pass' :`fail(${r.pctBelow.toFixed(2)}%)`,          `TC-30: amount diff% must be < 6%`).toBe('pass');
        ...
        expect.soft(c.tradeHistoryExcluded ? 'not-in-th' : 'in-th',                        `TC-30: Pending sell order ${r.orderId} should NOT be in Trade History`).toBe('not-in-th');
        expect.soft(c.balanceOk ? 'pass' : `fail(avlb=${c.balanceNow} exp≈${c.expectedBalance} diff=${c.balanceDiff})`, 'TC-30: sellAvlb should decrease by locked amount').toBe('pass');
        console.log(`[TC-30] 2nd Pending Sell | Price: ${sellPrice2} | Amount: ${r.actual} | Open: ${c.openCount} | sellAvlb: ${c.balanceNow} | OrderId: ${r.orderId}`);
    });
    ...
    // ── TC-32 ─────────────────────────────────────────────────────────────────
    test('TC-32: balance shows buy orders locked in buyAvlb inOrder and sell orders locked in sellAvlb inOrder', async () => {
        ...
        for (const r of results) expect.soft(r.pass, r.msg).toBe(true);
        console.log(`[TC-32] Multi-order lock validated | Buy orders: ${multiOrderActualBuyTotals.length} | Sell orders: ${multiOrderActualSellAmounts.length}`);
    });
    ...
    test('TC-40: balance fully restored after Cancel All — inOrder returns to 0, both coins restored', async () => {
        ...
        for (const r of results) expect.soft(r.pass, r.msg).toBe(true);
        console.log(`[TC-40] Balance restored after Cancel All | BuyAvlb before: ${snapshotBeforeMultiOrders!.buyAvlb} | SellAvlb before: ${snapshotBeforeMultiOrders!.sellAvlb}`);
    });

    // ── TC-41 ─────────────────────────────────────────────────────────────────
    test('TC-41: capture full balance snapshot before placing above-market order', async () => {
        ...
        console.log(`[TC-41] Pre-above-market snapshot captured | Buy Avlb: ${snapshotBeforeAboveMarket.buyAvlb} | Sell Avlb: ${snapshotBeforeAboveMarket.sellAvlb}`);
    });
    ...
    // ── TC-42 ─────────────────────────────────────────────────────────────────
    test('TC-42: place buy limit order above market price — executes immediately at market', async () => {
        ...
        console.log(`[TC-42] Above-market buy limit order placed | Limit Price: ${r.limitPrice} | Executed Price: ${r.executedPrice} | Executed Amount: ${r.amount} | Success Message: "${r.successMsg}"`);
    });

    // ── TC-43 ─────────────────────────────────────────────────────────────────
    test('TC-43: above-market order is NOT in Open Orders (filled immediately)', async () => {
        test.skip(!orderSucceeded, 'Order not placed — insufficient balance or order failed. Skipping TC-43.');
        if (!aboveMarketOrderDetails) { console.warn('[TC-43] No above-market order data'); return; }
        ...
        console.log(`[TC-43] Above-market order in Open Orders | Is Absent: ${isAbsent} | Is Filled: ${isFilled} | Row Text: "${rowText.slice(0, 80)}"`);
    });

    // ── TC-44 ─────────────────────────────────────────────────────────────────
    test('TC-44: All Orders shows correct limit price and executed price for above-market order', async () => {
        test.skip(!orderSucceeded, 'Order not placed — insufficient balance or order failed. Skipping TC-44.');
        if (!aboveMarketOrderDetails) { console.warn('[TC-44] No above-market order data'); return; }
        ...
        expect.soft(r.sideText?.toLowerCase(), `TC-44: All Orders — above-market side should contain "${(tradeData.aboveMarketSide ?? 'Buy Full').toLowerCase()}"`).toContain((tradeData.aboveMarketSide ?? 'Buy Full').toLowerCase());
        console.log(`[TC-44] All Orders above-market | LimitPrice: ${r.rowLimitPrice} | ExecPrice: ${r.rowExecutedPrice} | Status: "${r.rowStatus}" | ColorOk: ${r.statusColorOk} | Side: "${r.sideText}"`);
    });

    // ── TC-45 ─────────────────────────────────────────────────────────────────
    test('TC-45: Trade History shows the above-market buy order with side "buy"', async () => {
        test.skip(!orderSucceeded, 'Order not placed — insufficient balance or order failed. Skipping TC-45.');
        if (!aboveMarketOrderDetails) { console.warn('[TC-45] No above-market order data'); return; }
        ...
        const thDetails: SpotOrderDetails = {
            ...
            orderId,
            // Above-market fills cross the book immediately — that's a taker fill.
        ...
        if (!r.orderId || !thDetails.orderId || r.orderId !== thDetails.orderId) {
            test.info().annotations.push({ type: 'suggestion', description: `TC-45: Above-market order "${thDetails.orderId}" not yet in Trade History — may still be processing.` });
            console.log(`[TC-45] Trade History — above-market order not found. Found orderId: "${r.orderId}", expected: "${thDetails.orderId}"`);
            return;
        }
        ...
        console.log(`[TC-45] Trade History above-market buy | Side: ${r.sideActual} | Executed: ${r.executedActual} | Amount: ${r.amountActual} | Total: ${r.totalActual} | OrderId: ${r.orderId}`);
    });
    ...
    test('TC-46: My Trades shows executed price (market price) — NOT the limit price', async () => {
        ...
        if (!aboveMarketOrderDetails) { console.warn('[TC-46] No above-market order data'); return; }
    ...
    // ── TC-47 ─────────────────────────────────────────────────────────────────
    test('TC-47: balance after above-market fill — quote coin decreased, base coin increased', async () => {
        test.skip(!orderSucceeded, 'Order not placed — insufficient balance or order failed. Skipping TC-47.');
        if (!snapshotBeforeAboveMarket || !aboveMarketOrderDetails) { console.warn('[TC-47] Missing data'); return; }
        ...
        const results = await spotBuyPage.validateMarketFillBalance(
            ...
            executedPrice, amount, limitPrice, quoteCoin, baseCoin, 'buy', parseFloat(tradeData.takerFeePercent ?? '0'),
        );
        for (const r of results) expect.soft(r.pass, r.msg).toBe(true);
        console.log(`[TC-47] Balance after above-market fill | Executed Price: ${executedPrice} | Executed Amount: ${amount} | LimitPrice: ${limitPrice}`);
    });
