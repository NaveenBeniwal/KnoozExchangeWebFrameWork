...
test.describe.serial('Spot Module — Sell Limit Order Positive Flow', () => {
    ...
    // ── TC-03 ─────────────────────────────────────────────────────────────────
    test('TC-03: search currency pair in dropdown', async () => {
    ...
    test('TC-05: unmark currency pair from favorites', async () => {
        ...
        console.log(`[TC-05] Unmarked ${tradeData.searchPair} from favorites | Status: "${r.favoriteRemovedStatus}" | Message: "${r.favoriteMsg}"`);
    });
    ...
    test('TC-08: order book column headers show Price, Amount and Total', async () => {
        ...
        console.log(`[TC-28] Order book headers | Price: "${h.price}" | Amount: "${h.amount}" | Total: "${h.total}"`);
    });
    ...
    // ── TC-12 ─────────────────────────────────────────────────────────────────
    test('TC-12: order book has actual ask and bid rows', async () => {
    ...
    // ── TC-13 ─────────────────────────────────────────────────────────────────
    test('TC-13: top bid < top ask (valid spread)', async () => {
    ...
    // ── TC-16 ─────────────────────────────────────────────────────────────────
    test('TC-16: precision change alters LTP decimal display', async () => {
    ...
    // ── TC-17 ─────────────────────────────────────────────────────────────────
    test('TC-17: clicking order book bid rows (5 rows) pre-fills Price and Amount inputs', async () => {
    ...
    // ── TC-18 ─────────────────────────────────────────────────────────────────
    test('TC-18: Sell button label shows "SELL {baseCoin}"', async () => {
        await spotSellPage.selectLimitSellTab();
    ...
    // ── TC-19 ─────────────────────────────────────────────────────────────────
    test('TC-19: Buy tab shows quote currency available balance', async () => {
    ...
    // ── TC-20–TC-23: % buttons ──────────────────────────────────────────────
    test('TC-20: 25% button fills correct amount of base coin', async () => {
        await spotSellPage.selectLimitSellTab();
    ...
    test('TC-21: 50% button fills correct amount', async () => {
        const avlb   = await spotSellPage.fetchAvailableBalance();
    ...
    test('TC-22: 75% button fills correct amount', async () => {
        const avlb   = await spotSellPage.fetchAvailableBalance();
    ...
    test('TC-23: 100% button fills correct amount', async () => {
        const avlb   = await spotSellPage.fetchAvailableBalance();
    ...
    // ── TC-24 ─────────────────────────────────────────────────────────────────
    test('TC-24: pair header displays the selected pair name', async () => {
    ...
    // ── TC-25 ─────────────────────────────────────────────────────────────────
    test('TC-25: capture full balance snapshot before placing sell order', async () => {
    ...
    // ── TC-26 ─────────────────────────────────────────────────────────────────
    test('TC-26: enter sell limit price and amount, verify estimated fee', async () => {
        const price      = parseFloat(tradeData.sellLimitPrice);
    ...
    // ── TC-28 ─────────────────────────────────────────────────────────────────
    test('TC-28: base coin available balance before placing order', async () => {
    ...
    // ── TC-43 ─────────────────────────────────────────────────────────────────
    test('TC-43: capture full balance snapshot before placing below-market sell', async () => {
        ...
        console.log(`[TC-43] Pre-below-market snapshot captured | Sell Avlb: ${snapshotBeforeBelowMarket.sellAvlb} | Buy Avlb: ${snapshotBeforeBelowMarket.buyAvlb}`);
    });
    ...
    // ── TC-44 ─────────────────────────────────────────────────────────────────
    test('TC-44: sell limit below market price — executes immediately at market', async () => {
        ...
        console.log(`[TC-44] Below-market sell limit order placed | Limit Price: ${r.limitPrice} | Executed Price: ${r.executedPrice} | Executed Amount: ${r.amount} | Success Message: "${r.successMsg}"`);
    });

    // ── TC-45 ─────────────────────────────────────────────────────────────────
    test('TC-45: below-market sell NOT in Open Orders (filled immediately)', async () => {
        ...
        console.log(`[TC-45] Below-market sell in Open Orders | Is Absent: ${isAbsent} | Is Filled: ${isFilled} | Row Text: "${rowText.slice(0, 80)}"`);
    });

    // ── TC-46 ─────────────────────────────────────────────────────────────────
    test('TC-46: All Orders shows correct limit price and executed price', async () => {
        ...
        expect.soft(r.rowLimitPrice,    `TC-46: All Orders — limit price (expected ≈ ${limitPrice})`).toBeGreaterThan(0);
        expect.soft(r.rowExecutedPrice, `TC-46: All Orders — executed price should be > 0`).toBeGreaterThan(0);
        expect.soft(r.rowStatus,        `TC-46: All Orders — status should be "${tradeData.doneStatus ?? 'Done'}"`).toBe(tradeData.doneStatus ?? 'Done');
        expect.soft(r.statusColorOk ? 'green' : 'not-green', 'TC-46: Done status color should be green').toBe('green');
        expect.soft(r.sideText?.toLowerCase(), `TC-46: All Orders — below-market side should contain "${(tradeData.belowMarketSide ?? 'Sell Full').toLowerCase()}"`).toContain((tradeData.belowMarketSide ?? 'Sell Full').toLowerCase());
        console.log(`[TC-46] All Orders below-market sell | LimitPrice: ${r.rowLimitPrice} | ExecPrice: ${r.rowExecutedPrice} | Status: "${r.rowStatus}" | ColorOk: ${r.statusColorOk} | Side: "${r.sideText}"`);
    });

    // ── TC-47 ─────────────────────────────────────────────────────────────────
    test('TC-47: Trade History shows the below-market sell order with side "sell"', async () => {
        test.skip(!orderSucceeded, 'Order not placed — insufficient balance or order failed. Skipping TC-47.');
        if (!belowMarketOrderDetails) { console.warn('[TC-47] No below-market order data'); return; }
        ...
        if (!r.orderId || !thDetails.orderId || r.orderId !== thDetails.orderId) {
            test.info().annotations.push({ type: 'suggestion', description: `TC-47: Below-market order "${thDetails.orderId}" not yet in Trade History — may still be processing.` });
            console.log(`[TC-47] Trade History — below-market order not found. Found orderId: "${r.orderId}", expected: "${thDetails.orderId}"`);
            return;
        }
        ...
        console.log(`[TC-47] Trade History below-market sell | Side: ${r.sideActual} | Executed: ${r.executedActual} | Amount: ${r.amountActual} | Total: ${r.totalActual} | OrderId: ${r.orderId}`);
    });

    // ── TC-48 ─────────────────────────────────────────────────────────────────
    test('TC-48: My Trades shows executed price (market price) — NOT the limit price', async () => {
        test.skip(!orderSucceeded, 'Order not placed — insufficient balance or order failed. Skipping TC-48.');
        if (!belowMarketOrderDetails) { console.warn('[TC-48] No below-market order data'); return; }
        ...
        if (entries.length > 0) {
            ...
            console.log(`[TC-48] My Trades | Executed Price: ${entry.price} | Amount: ${entry.amount} | Time: "${entry.time}" | Limit Price: ${limitPrice} | Expected Executed: ${executedPrice}`);
        } else {
            console.warn('[TC-48] My Trades — no entries found');
        }
    });

    // ── TC-49 ─────────────────────────────────────────────────────────────────
    test('TC-49: balance after fill — base coin decreased, quote coin increased', async () => {
        test.skip(!orderSucceeded, 'Order not placed — insufficient balance or order failed. Skipping TC-49.');
        if (!snapshotBeforeBelowMarket || !belowMarketOrderDetails) { console.warn('[TC-49] Missing data'); return; }
        ...
        for (const r of results) expect.soft(r.pass, r.msg).toBe(true);
        console.log(`[TC-49] Balance after below-market sell fill | Executed Price: ${executedPrice} | Executed Amount: ${amount} | LimitPrice: ${limitPrice}`);
    });
    ...
    // ── TC-50 ─────────────────────────────────────────────────────────────────
    test('TC-50: snapshot before placing multiple pending orders (Cancel All test)', async () => {
        ...
        console.log(`[TC-50] Pre-multi-order snapshot | Sell Avlb: ${snapshotBeforeMultiOrders.sellAvlb} | Buy Avlb: ${snapshotBeforeMultiOrders.buyAvlb}`);
    });

    // ── TC-51: 1st pending sell limit order ───────────────────────────────
    test('TC-51: place 1st pending sell limit order and verify state after placement', async () => {
        test.skip(!orderSucceeded, 'Prior order not placed — skipping TC-51.');
        const price2  = parseFloat(tradeData.sellLimitPrice2  ?? '');
        const amount2 = parseFloat(tradeData.sellLimitAmount2 ?? '');
        test.skip(!price2 || !amount2, 'sellLimitPrice2 or sellLimitAmount2 missing in CSV — skipping TC-51.');
        ...
        expect.soft(r.successMsg,   'TC-51: 1st pending sell order should succeed').toContain('success');
        expect.soft(r.amountOk ? 'pass' :`fail(${r.actual} > ${amount2 * 1.001})`, `TC-51: All Orders remaining must not exceed entered (${amount2})`).toBe('pass');
        expect.soft(r.pctBelowOk ? 'pass' :`fail(${r.pctBelow.toFixed(2)}%)`,     `TC-51: amount diff% must be < 6%`).toBe('pass');
        ...
        expect.soft(c.allOrdersStatus.toLowerCase(),                                   'TC-51: All Orders status should be New/pending').toMatch(/new|pending|open/i);
        expect.soft(c.openCount,                                                        'TC-51: Open Orders should show ≥ 1').toBeGreaterThanOrEqual(1);
        expect.soft(c.tradeHistoryExcluded ? 'not-in-th' : 'in-th',                    `TC-51: Pending order ${r.orderId} should NOT be in Trade History`).toBe('not-in-th');
        expect.soft(c.balanceOk ? 'pass' : `fail(avlb=${c.balanceNow} exp≈${c.expectedBalance} diff=${c.balanceDiff})`, 'TC-51: sellAvlb should decrease by locked amount').toBe('pass');
        console.log(`[TC-51] 1st Pending Sell | Price: ${price2} | Amount: ${r.actual} | Open: ${c.openCount} | sellAvlb: ${c.balanceNow} | OrderId: ${r.orderId}`);
    });

    // ── TC-52: 2nd pending sell limit order ────────────────────────────────
    test('TC-52: place 2nd pending sell limit order and verify state after placement', async () => {
        test.skip(!multiOrdersSucceeded, 'Previous order failed — skipping TC-52.');
        const price3  = parseFloat(tradeData.sellLimitPrice3  ?? '');
        const amount3 = parseFloat(tradeData.sellLimitAmount3 ?? '');
        test.skip(!price3 || !amount3, 'sellLimitPrice3 or sellLimitAmount3 missing in CSV — skipping TC-52.');
        const r = await spotSellPage.placePendingSellLimitOrder(price3, amount3);
        multiOrderActualSellAmounts.push(r.actual);
        expect.soft(r.successMsg,   'TC-52: 2nd pending sell order should succeed').toContain('success');
        expect.soft(r.amountOk ? 'pass' :`fail(${r.actual} > ${amount3 * 1.001})`, `TC-52: All Orders remaining must not exceed entered (${amount3})`).toBe('pass');
        expect.soft(r.pctBelowOk ? 'pass' :`fail(${r.pctBelow.toFixed(2)}%)`,     `TC-52: amount diff% must be < 6%`).toBe('pass');
        ...
        expect.soft(c.allOrdersStatus.toLowerCase(),                                   'TC-52: All Orders status should be New/pending').toMatch(/new|pending|open/i);
        expect.soft(c.openCount,                                                        'TC-52: Open Orders should show ≥ 2').toBeGreaterThanOrEqual(2);
        expect.soft(c.tradeHistoryExcluded ? 'not-in-th' : 'in-th',                    `TC-52: Pending order ${r.orderId} should NOT be in Trade History`).toBe('not-in-th');
        expect.soft(c.balanceOk ? 'pass' : `fail(avlb=${c.balanceNow} exp≈${c.expectedBalance} diff=${c.balanceDiff})`, 'TC-52: sellAvlb should decrease by locked amount').toBe('pass');
        console.log(`[TC-52] 2nd Pending Sell | Price: ${price3} | Amount: ${r.actual} | Open: ${c.openCount} | sellAvlb: ${c.balanceNow} | OrderId: ${r.orderId}`);
    });

    // ── TC-53: 3rd pending sell limit order ────────────────────────────────
    test('TC-53: place 3rd pending sell limit order and verify state after placement', async () => {
        test.skip(!multiOrdersSucceeded, 'Previous order failed — skipping TC-53.');
        const price4  = parseFloat(tradeData.sellLimitPrice4  ?? '');
        const amount4 = parseFloat(tradeData.sellLimitAmount4 ?? '');
        test.skip(!price4 || !amount4, 'sellLimitPrice4 or sellLimitAmount4 missing in CSV — skipping TC-53.');
        const r = await spotSellPage.placePendingSellLimitOrder(price4, amount4);
        multiOrderActualSellAmounts.push(r.actual);
        expect.soft(r.successMsg,   'TC-53: 3rd pending sell order should succeed').toContain('success');
        expect.soft(r.amountOk ? 'pass' :`fail(${r.actual} > ${amount4 * 1.001})`, `TC-53: All Orders remaining must not exceed entered (${amount4})`).toBe('pass');
        expect.soft(r.pctBelowOk ? 'pass' :`fail(${r.pctBelow.toFixed(2)}%)`,     `TC-53: amount diff% must be < 6%`).toBe('pass');
        ...
        expect.soft(c.allOrdersStatus.toLowerCase(),                                   'TC-53: All Orders status should be New/pending').toMatch(/new|pending|open/i);
        expect.soft(c.openCount,                                                        'TC-53: Open Orders should show ≥ 3').toBeGreaterThanOrEqual(3);
        expect.soft(c.tradeHistoryExcluded ? 'not-in-th' : 'in-th',                    `TC-53: Pending order ${r.orderId} should NOT be in Trade History`).toBe('not-in-th');
        expect.soft(c.balanceOk ? 'pass' : `fail(avlb=${c.balanceNow} exp≈${c.expectedBalance} diff=${c.balanceDiff})`, 'TC-53: sellAvlb should decrease by locked amount').toBe('pass');
        console.log(`[TC-53] 3rd Pending Sell | Price: ${price4} | Amount: ${r.actual} | Open: ${c.openCount} | sellAvlb: ${c.balanceNow} | OrderId: ${r.orderId}`);
    });

    // ── TC-54: 4th pending sell limit order ────────────────────────────────
    test('TC-54: place 4th pending sell limit order and verify state after placement', async () => {
        test.skip(!multiOrdersSucceeded, 'Previous order failed — skipping TC-54.');
        const price5  = parseFloat(tradeData.sellLimitPrice5  ?? '');
        const amount5 = parseFloat(tradeData.sellLimitAmount5 ?? '');
        test.skip(!price5 || !amount5, 'sellLimitPrice5 or sellLimitAmount5 missing in CSV — skipping TC-54.');
        const r = await spotSellPage.placePendingSellLimitOrder(price5, amount5);
        multiOrderActualSellAmounts.push(r.actual);
        expect.soft(r.successMsg,   'TC-54: 4th pending sell order should succeed').toContain('success');
        expect.soft(r.amountOk ? 'pass' :`fail(${r.actual} > ${amount5 * 1.001})`, `TC-54: All Orders remaining must not exceed entered (${amount5})`).toBe('pass');
        expect.soft(r.pctBelowOk ? 'pass' :`fail(${r.pctBelow.toFixed(2)}%)`,     `TC-54: amount diff% must be < 6%`).toBe('pass');
        ...
        expect.soft(c.allOrdersStatus.toLowerCase(),                                   'TC-54: All Orders status should be New/pending').toMatch(/new|pending|open/i);
        expect.soft(c.openCount,                                                        'TC-54: Open Orders should show ≥ 4').toBeGreaterThanOrEqual(4);
        expect.soft(c.tradeHistoryExcluded ? 'not-in-th' : 'in-th',                    `TC-54: Pending order ${r.orderId} should NOT be in Trade History`).toBe('not-in-th');
        expect.soft(c.balanceOk ? 'pass' : `fail(avlb=${c.balanceNow} exp≈${c.expectedBalance} diff=${c.balanceDiff})`, 'TC-54: sellAvlb should decrease by locked amount').toBe('pass');
        console.log(`[TC-54] 4th Pending Sell | Price: ${price5} | Amount: ${r.actual} | Open: ${c.openCount} | sellAvlb: ${c.balanceNow} | OrderId: ${r.orderId}`);
    });

    // ── TC-55: 1st pending buy limit order (for cancel-all) ────────────────
    test('TC-55: place 1st pending buy limit order and verify state after placement', async () => {
        test.skip(!multiOrdersSucceeded, 'Previous order failed — skipping TC-55.');
        const buyPrice1 = parseFloat(tradeData.cancelAllBuyPrice1 ?? '');
        const buyTotal1 = parseFloat(tradeData.cancelAllBuyTotal1 ?? '');
        test.skip(!buyPrice1 || !buyTotal1, 'cancelAllBuyPrice1 or cancelAllBuyTotal1 missing in CSV — skipping TC-55.');
        ...
        expect.soft(r.successMsg,  'TC-55: 1st pending buy order should succeed').toContain('success');
        expect.soft(r.totalOk ? 'pass' :`fail(${r.actual} > ${buyTotal1 * 1.001})`, `TC-55: All Orders total must not exceed entered (${buyTotal1})`).toBe('pass');
        expect.soft(r.pctBelowOk ? 'pass' :`fail(${r.pctBelow.toFixed(2)}%)`,      `TC-55: total diff% must be < 6%`).toBe('pass');
        ...
        expect.soft(c.allOrdersStatus.toLowerCase(),                                    'TC-55: All Orders status should be New/pending').toMatch(/new|pending|open/i);
        expect.soft(c.openCount,                                                         'TC-55: Open Orders should show ≥ 5').toBeGreaterThanOrEqual(5);
        expect.soft(c.tradeHistoryExcluded ? 'not-in-th' : 'in-th',                     `TC-55: Pending buy order ${r.orderId} should NOT be in Trade History`).toBe('not-in-th');
        expect.soft(c.balanceOk ? 'pass' : `fail(avlb=${c.balanceNow} exp≈${c.expectedBalance} diff=${c.balanceDiff})`, 'TC-55: buyAvlb should decrease by locked amount').toBe('pass');
        console.log(`[TC-55] 1st Pending Buy | Price: ${buyPrice1} | Total: ${r.actual} | Open: ${c.openCount} | buyAvlb: ${c.balanceNow} | OrderId: ${r.orderId}`);
    });

    // ── TC-56: 2nd pending buy limit order (for cancel-all) ────────────────
    test('TC-56: place 2nd pending buy limit order and verify state after placement', async () => {
        test.skip(!multiOrdersSucceeded, 'Previous order failed — skipping TC-56.');
        const buyPrice2 = parseFloat(tradeData.cancelAllBuyPrice2 ?? '');
        const buyTotal2 = parseFloat(tradeData.cancelAllBuyTotal2 ?? '');
        test.skip(!buyPrice2 || !buyTotal2, 'cancelAllBuyPrice2 or cancelAllBuyTotal2 missing in CSV — skipping TC-56.');
        const r = await spotSellPage.placePendingBuyLimitOrderForSellSpec(buyPrice2, buyTotal2);
        multiOrderActualBuyTotals.push(r.actual);
        expect.soft(r.successMsg,  'TC-56: 2nd pending buy order should succeed').toContain('success');
        expect.soft(r.totalOk ? 'pass' :`fail(${r.actual} > ${buyTotal2 * 1.001})`, `TC-56: All Orders total must not exceed entered (${buyTotal2})`).toBe('pass');
        expect.soft(r.pctBelowOk ? 'pass' :`fail(${r.pctBelow.toFixed(2)}%)`,      `TC-56: total diff% must be < 6%`).toBe('pass');
        ...
        expect.soft(c.allOrdersStatus.toLowerCase(),                                    'TC-56: All Orders status should be New/pending').toMatch(/new|pending|open/i);
        expect.soft(c.openCount,                                                         'TC-56: Open Orders should show ≥ 6').toBeGreaterThanOrEqual(6);
        expect.soft(c.tradeHistoryExcluded ? 'not-in-th' : 'in-th',                     `TC-56: Pending buy order ${r.orderId} should NOT be in Trade History`).toBe('not-in-th');
        expect.soft(c.balanceOk ? 'pass' : `fail(avlb=${c.balanceNow} exp≈${c.expectedBalance} diff=${c.balanceDiff})`, 'TC-56: buyAvlb should decrease by locked amount').toBe('pass');
        console.log(`[TC-56] 2nd Pending Buy | Price: ${buyPrice2} | Total: ${r.actual} | Open: ${c.openCount} | buyAvlb: ${c.balanceNow} | OrderId: ${r.orderId}`);
    });

    // ── TC-57 ─────────────────────────────────────────────────────────────────
    test('TC-57: Open Orders shows all 6 pending orders and Cancel All button', async () => {
        test.skip(!multiOrdersSucceeded || !snapshotBeforeMultiOrders, 'No orders or no snapshot — skipping TC-57.');
        ...
        console.log(`[TC-57] Open Orders row count: ${r.rowCount} | Cancel All visible: ${r.cancelAllVisible}`);
    });

    // ── TC-58 ─────────────────────────────────────────────────────────────────
    test('TC-58: balance shows sell orders locked in sellAvlb inOrder and buy orders locked in buyAvlb inOrder', async () => {
        test.skip(!multiOrdersSucceeded || !snapshotBeforeMultiOrders, 'No orders or no snapshot — skipping TC-58.');
        ...
        for (const r of results) expect.soft(r.pass, r.msg).toBe(true);
        console.log(`[TC-58] Multi-order lock validated | Sell orders: ${multiOrderActualSellAmounts.length} | Buy orders: ${multiOrderActualBuyTotals.length}`);
    });

    // ── TC-59 ─────────────────────────────────────────────────────────────────
    test('TC-59: Trade History does NOT show pending/open limit orders', async () => {
        test.skip(!multiOrdersSucceeded, 'No multi-orders — skipping TC-59.');
        ...
        console.log(`[TC-59] Trade History row count after multi-orders: ${rows} (pending orders should not appear here)`);
        test.info().annotations.push({ type: 'info', description: `TC-59: Trade History checked — pending limit orders should not appear. Rows found: ${rows}` });
    });

    // ── TC-60: cancel one order individually ───────────────────────────────
    test('TC-60: cancel one order individually by clicking its row Cancel button', async () => {
        test.skip(!multiOrdersSucceeded, 'No multi-orders placed — skipping TC-60.');
        await spotSellPage.openOrdersTab.click();
        await page.waitForTimeout(1000);
        const result = await spotSellPage.cancelFirstOpenOrder();
        expect.soft(result.confirmed ? 'cancelled' : 'not cancelled', `TC-60: Individual cancel toast — ${result.toastMsg}`).toBe('cancelled');
        console.log(`[TC-60] Individual cancel | Confirmed: ${result.confirmed} | Toast: "${result.toastMsg}"`);
    });

    // ── TC-61 ─────────────────────────────────────────────────────────────────
    test('TC-61: 5 orders remain in Open Orders after individual cancel', async () => {
        test.skip(!multiOrdersSucceeded, 'No multi-orders — skipping TC-61.');
        await spotSellPage.openOrdersTab.click();
        await page.waitForTimeout(1000);
        ...
        console.log(`[TC-61] Open Orders after individual cancel: ${remaining} rows`);
    });

    // ── TC-62: Cancel All ──────────────────────────────────────────────────
    test('TC-62: Cancel All button removes all remaining open orders', async () => {
        test.skip(!multiOrdersSucceeded, 'No multi-orders placed — skipping TC-62.');
        ...
        expect.soft(result.confirmed ? 'cancelled' : 'not cancelled', `TC-62: Cancel All — ${result.toastMsg}`).toBe('cancelled');
        expect.soft(v.match, `TC-62: Cancel All toast should contain "${tradeData.cancelAllMsg ?? 'cancel/success'}"`).toBe('match');
        console.log(`[TC-62] Cancel All | Confirmed: ${result.confirmed} | Toast: "${result.toastMsg}" | ExpectedMsg: "${tradeData.cancelAllMsg ?? '(none)'}"`);
    });

    // ── TC-63 ─────────────────────────────────────────────────────────────────
    test('TC-63: Open Orders shows "No data" after Cancel All', async () => {
        test.skip(!multiOrdersSucceeded, 'No multi-orders — skipping TC-63.');
        ...
        console.log(`[TC-63] Open Orders after Cancel All | No Data visible: ${r.isVisible} | Row count: ${r.rowCount}`);
    });

    // ── TC-64 ─────────────────────────────────────────────────────────────────
    test('TC-64: All Orders shows cancelled/done status for the cancelled orders', async () => {
        test.skip(!multiOrdersSucceeded, 'No multi-orders placed — skipping TC-64.');
        const r = await spotSellPage.validateAllOrdersCancelled(tradeData.searchPair);
        expect.soft(r.statusResult,                          'TC-64: All Orders row after Cancel All should show cancelled/done status').toBe('cancelled/done');
        expect.soft(r.cancelledColorOk ? 'red' : 'not-red', 'TC-64: Cancelled status color should be red').toBe('red');
        console.log(`[TC-64] All Orders after Cancel All | Status: "${r.statusActual}" | CancelledColorOk: ${r.cancelledColorOk} | Pair: ${r.pairActual}`);
    });

    // ── TC-65 ─────────────────────────────────────────────────────────────────
    test('TC-65: Trade History does not show the cancelled pending orders', async () => {
        test.skip(!multiOrdersSucceeded, 'No multi-orders placed — skipping TC-65.');
        const r = await spotSellPage.validateTradeHistoryAfterCancelAll();
        test.info().annotations.push({ type: 'info', description: `TC-65: Trade History rows after Cancel All: ${r.rowCount}. Cancelled pending orders should not appear here.` });
        console.log(`[TC-65] Trade History after Cancel All | Row count: ${r.rowCount} (cancelled pending orders should not be here)`);
    });

    // ── TC-66 ─────────────────────────────────────────────────────────────────
    test('TC-66: balance fully restored after Cancel All — inOrder returns to 0, both coins restored', async () => {
        test.skip(!multiOrdersSucceeded || !snapshotBeforeMultiOrders, 'No orders or no pre-snapshot — skipping TC-66.');
        ...
        for (const r of results) expect.soft(r.pass, r.msg).toBe(true);
        console.log(`[TC-66] Balance restored after Cancel All | SellAvlb before: ${snapshotBeforeMultiOrders!.sellAvlb} | BuyAvlb before: ${snapshotBeforeMultiOrders!.buyAvlb}`);
    });
